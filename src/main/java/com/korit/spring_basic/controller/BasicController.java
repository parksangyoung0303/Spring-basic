...
public class BasicController {
    ...
    // POST method - 클라이언트가 서버에 데이터를 전송하기위한 메서드
    //             - 클라이언트가 서버에 데이터를 작성하기위한 메서드
    ...
    // PUT method - 클라이언트가 서버에 데이터를 생성 or 수정하기 위한 메서드
    //            - 만약 전송한 데이터에 동일한 데이터가 있다면 수정, 만약 전송한 데이터가 존재하지 않는다면 생성
    ...
    // @PatchMapping : Request Mapping을 PATCH에 한정시킨 어노테이션
    ...
    // @DeleteMapping : Request Mapping을 DELETE에 한정시킨 어노테이션
    // DELETE method - 클라이언트가 서버에 데이터를 삭제하기 위한 메서드
    // Request Body X / Response Body X / HTML form O
    @DeleteMapping("")
