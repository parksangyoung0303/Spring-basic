...
import com.korit.spring_basic.service.BasicService;
...
public class ServiceController {
    ...
    // 의존성 주입 (DI) : 해당 모듈에 필요한 의존성을 클래스의 인스턴스를 생성하는 위치에서 주입
    // - Spring framwork에서 @Autowired을 이용하여 의존성 주입을 Spring에게 위임할 수 있음 (IoC, DI)
    // - 필드를 이용한 의존성 주입, setter 메서드를 이용한 의존성 주입, 생성자를 이용한 의존성 주입
    // - 생성자를 이용한 의존성 주입 방법이 가장 안정적이므로 Spring Framwork에서는 생성자를 이용한 의존성 주입을 권장
    // ! 주의: IoC를 통해 Spring에게 의존성 주입을 위임하려면 주입하려는 클래스가 Spring Bean으로 등록(@Component) 되어 있어야 함
