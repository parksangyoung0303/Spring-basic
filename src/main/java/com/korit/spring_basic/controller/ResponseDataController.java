...
public class ResponseDataController {
    ...
    @GetMapping("/{name}")
    public ResponseDto getResponseDto(@PathVariable("name") String name) {
    ...
    // - ResponseEntity의 제너릭을 이용해서 Response Body의 타입을 지정해야함

    @GetMapping("/response-entity/{name}")
    public ResponseEntity<ResponseDto> responseEntity(@PathVariable String name) {
        ResponseDto response = new ResponseDto(name, new Date());
