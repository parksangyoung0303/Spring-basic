...
// - spring framwork에서 제공하는 유효성 검사 인터페이스 라이브러리
// - client가 서버로 데이터를 전송할 때 유효한 데이터일 떄만 서버에 데이터를 전송할 수 있도록 도움을 줌
...
public class Validation {
    ...
    // ! 주의: NotBlank and NotEmpty를 문자열이 아닌 타입에서 사용하지 않도록 해야함
    // @NotBlank
    ...
    // @Pattern(regexp= 정규식): 정규식을 이용해서 문자열의 패턴을 검사할 수 있다
    // null에 대한 검사는 수행하지 않음
