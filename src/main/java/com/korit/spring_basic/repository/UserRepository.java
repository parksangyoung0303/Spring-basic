...
public interface UserRepository extends JpaRepository<UserEntity, String> {
    ...
    // Query method: JpaRepository의 메서드 선언시에 지정된 규칙에 따라 메서드명을 작성하면 JPA가 SQL을 만들어주는 방법

    // - findBy [SELECT * WHERE]: 필드명을 기준으로 모든 컬럼을 조회할 때 사용, findBy 바로 뒤에 필드명을 붙여서 작성,
    //                            필드명은 반드시 첫글자가 대문자로 작성되어야함
    ...
    // - 표준 SQL과 매우 흡사 하지만 Entity 및 Entity의 필드로 쿼리를 작성
